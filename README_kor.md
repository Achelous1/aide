...
<p align="center">
  <img src="docs/brand/smalti-icon-source-1024.png" width="180" alt="smalti 로고" />
</p>
...
smalti는 단 하나의 핵심 아이디어를 중심으로 만들어진 Electron 기반 IDE입니다: **IDE가 사용자에게 맞춰져야지, 그 반대가 되어서는 안 된다**. 마켓플레이스에서 미리 만들어진 플러그인 수십 개를 설치하는 대신, 원하는 기능을 자연어로 설명하면 AI 에이전트가 즉시 동작하는 플러그인을 생성합니다. 생성된 플러그인은 샌드박스 환경에서 실행되며, AI 도구로 자동 등록되어 사용자와 AI 어시스턴트 모두가 즉시 사용할 수 있습니다.

<p align="center">
  <img src="docs/assets/screenshot-hero-overview.png" alt="smalti 전체 워크스페이스 — 멀티 패인 레이아웃" />
  <br/>
  <sub><em>일반적인 워크스페이스: 여러 에이전트 터미널, 코드 에디터 패인, 그리고 플러그인으로 렌더된 칸반 보드가 한 화면 안에 공존합니다.</em></sub>
</p>

smalti는 LLM API를 직접 호출하지 않습니다. Rust `portable-pty` 기반 네이티브 모듈을 통해 CLI 에이전트(`claude`, `gemini`, `codex`)를 PTY 프로세스로 spawn하므로, 각 에이전트가 자체 인증을 관리하고 사용자가 자신의 LLM 제공자 관계를 완전히 통제할 수 있습니다.
...
- **하나의 워크스페이스**에서 터미널, AI 에이전트, 파일 트리, 플러그인 UI를 모두 처리
...
| **핫 리로드** | 플러그인 코드, HTML, 디스크 추가가 smalti 재시작 없이 즉시 반영. AI가 새로 생성한 플러그인이 패널에 곧바로 표시됨 |
...
| **Rust 코어** | 파일 시스템, 워처, PTY 연산이 Rust `portable-pty` + `notify` 네이티브 모듈(napi-rs) 위에서 동작. 패키지 빌드 기준 idle CPU 약 127% → 약 0%. 한글 / 이모지 터미널 출력이 read 경계에서 깨지지 않음. |

<p align="center">
  <img src="docs/assets/screenshot-split-panes.png" alt="분할 패인 — 좌측 터미널, 우측 코드 에디터" />
  <br/>
  <sub><em>분할 레이아웃: 좌측 에이전트 세션, 우측 코드 편집. 탭을 드래그해 순서·배치를 실시간으로 바꿀 수 있습니다.</em></sub>
</p>
...
smalti 플러그인은 AI 에이전트와의 자연어 대화로 완전히 생성됩니다. 별도의 SDK도, 보일러플레이트 생성기도, 빌드 단계도 필요 없습니다.
...
에이전트: (MCP aide_create_plugin 도구 호출)
         ...
         → smalti 디자인 토큰을 사용한 index.html UI 생성
         → 에이전트가 이후 호출할 수 있도록 MCP 도구로 등록

smalti: 플러그인이 Plugins 패널에 즉시 표시됨. ON 토글로 활성화.
...
플러그인은 **워크스페이스 로컬** 범위로 존재합니다. 각 플러그인은 `<workspace>/.aide/plugins/<plugin-id>/` 아래에 위치하며 해당 워크스페이스에서만 보입니다. 런타임에 추가된 플러그인(MCP, 파일 매니저, 직접 복사 어느 방식이든)은 재시작 없이 자동 감지됩니다.

<p align="center">
  <img src="docs/assets/screenshot-plugin-kanban.png" alt="플러그인 UI — agent-todo-board 칸반이 패인으로 렌더링된 모습" />
  <br/>
  <sub><em>자연어로 생성된 플러그인(<code>agent-todo-board</code>)이 패인 하나를 가득 채워 렌더됩니다. 이 플러그인을 만든 에이전트 세션과 나란히 놓여 바로 피드백 루프가 가능합니다.</em></sub>
</p>
...
- **macOS** (Apple Silicon 또는 Intel) — Windows / Linux: 소스 설치 가능 (CI에서 3개 OS 모두 검증); 패키지 DMG 릴리즈는 현재 macOS 전용.
- **Node.js** ≥ 18
- **pnpm** (프로젝트가 `node-linker=hoisted`를 사용하므로 npm/yarn은 그대로는 동작하지 않음)
- **Rust** (rustup, stable ≥ 1.82) — Rust 코어 빌드에 필요. `pnpm install` 시 자동으로 빌드됩니다. rustup이 없으면: `curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh`. Homebrew rustc도 함께 설치된 경우, 빌드 스크립트가 `~/.cargo/bin`을 PATH 앞에 추가하므로 rustup이 우선 적용됩니다 — 별도 조치 불필요.

### CLI 에이전트 (선택)

smalti는 설치된 에이전트를 자동으로 감지합니다. 사용하는 것만 설치하면 됩니다:
...
빌드 스크립트는 의존성 설치, lint, 패키징, 드래그-드롭 인스톨러 레이아웃 DMG 생성을 모두 처리합니다. 또한 `pnpm run build:native:universal`을 실행해 lipo로 합쳐진 arm64 + x64 유니버설 바이너리를 생성하므로 DMG가 Apple Silicon과 Intel Mac 모두에서 동작합니다.

### macOS Gatekeeper (서명되지 않은 빌드)

배포용 DMG는 코드 서명이 되어 있지 않기 때문에, macOS가 첫 실행 시 격리(quarantine)시켜 "smalti가 손상되어 열 수 없습니다"라는 메시지가 표시될 수 있습니다. `/Applications`로 드래그해 설치한 뒤, 아래 명령어로 격리 속성을 제거해주세요:

```bash
xattr -c /Applications/AIDE.app
```

이후 정상적으로 smalti를 실행할 수 있습니다.

---

## 아키텍처

smalti는 엄격한 보안 경계를 갖는 Electron 3-프로세스 모델을 따릅니다.
...
│  └─────────────┘ └────────────┘ └──────────────────────┘ │
└──────────────────────────────────────────────────────────┘
  백엔드 연산(FS, 워처, PTY)은 crates/aide-core + crates/aide-napi →
  Main 프로세스가 로드하는 .node 모듈을 통해 처리됩니다.
```
...
<p align="center">
  <img src="docs/assets/screenshot-file-explorer.png" alt="파일 탐색기와 마크다운 프리뷰를 나란히" />
  <br/>
  <sub><em>파일 탐색기, 터미널, 마크다운 렌더링이 인접한 패인에 공존 — IDE를 떠나지 않고 프로젝트 문서를 탐색·편집할 수 있습니다.</em></sub>
</p>
...
| **상태 관리** | Zustand 5 |
| **터미널** | xterm.js + Rust portable-pty (napi-rs 경유) |
| **Rust 코어** | napi-rs (crates/aide-core, crates/aide-napi) — FS·워처·PTY |
...
smalti는 **Model Context Protocol** 서버를 내장하여 모든 MCP 호환 에이전트에게 플러그인 도구를 노출합니다.

### MCP 서버의 역할

smalti 시작 시 자체 완결형 MCP 서버 스크립트를 `~/.aide/aide-mcp-server.js`에 작성하고 각 에이전트의 글로벌 설정에 등록합니다. MCP 서버는 플러그인 JavaScript를 `node:vm`으로 실행하므로 Node.js 호스팅을 유지합니다.
