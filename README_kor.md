...
<p align="center">
  <img src="docs/brand/smalti-icon-source-1024.png" width="180" alt="AIDE — smalti로 리브랜드 진행 중" />
</p>
...
<p align="center">
  <img src="docs/assets/screenshot-hero-overview.png" alt="AIDE 전체 워크스페이스 — 멀티 패인 레이아웃" />
  <br/>
  <sub><em>일반적인 워크스페이스: 여러 에이전트 터미널, 코드 에디터 패인, 그리고 플러그인으로 렌더된 칸반 보드가 한 화면 안에 공존합니다.</em></sub>
</p>

AIDE는 LLM API를 직접 호출하지 않습니다. Rust `portable-pty` 기반 네이티브 모듈을 통해 CLI 에이전트(`claude`, `gemini`, `codex`)를 PTY 프로세스로 spawn하므로, 각 에이전트가 자체 인증을 관리하고 사용자가 자신의 LLM 제공자 관계를 완전히 통제할 수 있습니다.
...
AIDE 시작 시 자체 완결형 MCP 서버 스크립트를 `~/.aide/aide-mcp-server.js`에 작성하고 각 에이전트의 글로벌 설정에 등록합니다. MCP 서버는 플러그인 JavaScript를 `node:vm`으로 실행하므로 Node.js 호스팅을 유지합니다.
